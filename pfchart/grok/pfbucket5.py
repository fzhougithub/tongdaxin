...
def draw_pf_chart_with_volume(buckets: List[Dict], min_price: float, step: float, symbol: str):
    """Draw a Point and Figure chart with price and volume subplots per P&F bucket."""
    if not buckets:
    ...
    # Prepare data for volume chart (per P&F bucket)
    bucket_indices = list(range(len(buckets)))
    volumes = [bucket['volume'] for bucket in buckets]
    colors = ['blue' if b['mark'] == 'X' else 'red' for b in buckets]
    ...
    # Bottom subplot: Volume chart per P&F bucket
    ax2.bar(bucket_indices, volumes, color=colors)
...
def main(symbol: str, step: float = None):
    ...
    draw_pf_chart_with_volume(pf_chart, min_price, step if step else (max(day['c'] for day in stock_history) - min_price) / 50, symbol)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate and draw a Point and Figure chart with volume from stock history.")
    parser.add_argument("symbol", help="Stock symbol (e.g., '600050')")
    parser.add_argument("--step", type=float, default=None, help="Custom step size (optional; e.g., 0.25)")
