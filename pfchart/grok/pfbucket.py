...
def draw_pf_chart_with_volume(buckets: List[Dict], min_price: float, step: float, symbol: str):
    """Draw a Point and Figure chart with price and volume subplots."""
    if not buckets:
    ...
    volume_colors = ['red' if bucket['mark'] == 'X' else 'lightblue' for bucket in buckets]
    ...
    ax2.bar(bucket_indices, volumes, color=volume_colors)
