...
def draw_pf_chart(buckets: List[Dict], min_price: float, step: float, symbol: str):
    """Draw a Point and Figure chart using matplotlib."""
    if not buckets:
    ...
    # Create the plot
    plt.figure(figsize=(len(buckets) * 0.5, num_levels * 0.3))  # Adjust size dynamically
    for i, (x, y, mark) in enumerate(zip(x_coords, y_coords, marks)):
        plt.text(x, y, mark, fontsize=12, ha='center', va='center', 
                 color='blue' if mark == 'X' else 'red')

    # Customize the plot
    plt.xticks(range(len(buckets)), [f"B{i+1}" for i in range(len(buckets))])
    plt.yticks(range(len(price_levels)), [f"{p:.2f}" for p in price_levels])
    plt.xlabel("Buckets")
    plt.ylabel("Price Levels")
    plt.title(f"Point and Figure Chart for {symbol} (Step: {step})")
...
def main(symbol: str, step: float = None):
    ...
    # Calculate min_price for plotting
    min_price = min(day['c'] for day in stock_history)

    # Draw the chart
    draw_pf_chart(pf_chart, min_price, step if step else (max(day['c'] for day in stock_history) - min_price) / 50, symbol)

    # Print bucket details (optional)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate and draw a Point and Figure chart from stock history.")
